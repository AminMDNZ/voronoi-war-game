...
public class NewV extends JPanel {
    ...
    public int countPlayer = 3;
    public boolean gameSatart = false;
    public boolean gameEnd = false;

    public int countAttackCountry = 2;
    public int countArcherCountry = 3;
    public int countMuinusSelf = 2;
    ...
    //    public boolean[] useColor = new boolean[20];
    public static final int PANEL_HEIGHT = 700;
    public int NUM_PARTITIONS = 20;
    public List<Integer> playerCountryId = new ArrayList<>();
    public boolean isRandom = false;
    public static final int CENTER_RADIUS = 3;
    public Player player1;
    public StackedChartPanel stackedChartPanel;
    ...
    public NewV(int countPlayer , boolean isRandom , int timer , int countCity) {
         firstTime = timer;
        remainingTime = timer;
        this.NUM_PARTITIONS = countCity;
...
//                    repaint();
    ...
    public boolean checkGameEnd() {
        System.out.println("Remainin time: " + remainingTime);
        int countRemain = 0;
        if(player1.getCountries().size() == 0){
            return true;
        }
        for (int i = 0; i < Player.players.size(); i++) {
            if (Player.players.get(i).getCountries().size() > 0) {
                countRemain++;
                if(countRemain == 2) return false;
            }
        }
...
//            System.out.println("you win");
             message = "you catch " + player1.getCountries().size() + " countries. You 1st winner.";
        }
        else if(playerPercentage > 30){
             message = "you catch " + player1.getCountries().size() + " countries. You win.";
...
//                                                            Ball ball = new Ball();
    ...
    public Partition getPartitionAtPoint(Point point) {
    ...
    public void darkenPartitionColor(Partition partition) {
...
//            Color color = getRandomColor(random);
    ...
    public class StackedChartPanel extends JPanel {
    ...
    public Map<Color, Double> calculateColorPercentages() {
        Map<Color, Integer> colorCounts = new HashMap<>();
...
//            System.out.println("intiCountry: " + initCountry);
//
            if(initCountry){
...
//            }
    ...
    public ImageIcon getPartitionIcon(Partition partition) {
...
//            ImageIcon icon
    ...
    public Partition getNearestPartition(int x, int y) {
    ...
    public class Ball implements Runnable {
        ...
        public static final int UPDATE_INTERVAL = 16; // Update interval in milliseconds (approx. 60 FPS)
        ...
        public void damageToCountry(){
            if(country.getCathcerId() != attacker.getId()) {
                ...
                if(cachCountry) {
                    if(player1.getId() == attacker.getId()){
        ...
        public void run() {
            while (!shouldRemove) {
                ...
                remainingTime /= 1000;
                for(Partition partition : partitions){
                    partition.bowPower += 1;
                }
                int i = (int)startPoint.getX();
                int j = (int)startPoint.getY();
                Partition nearestPartition = getNearestPartition(i, j);
                int ind = getPartitionIndex(i, j);
                String type = Country.countryList.get(ind).getCountryType();


                System.out.println("bow Power: + " + nearestPartition.bowPower);
                if((type.equals("Archer")) && nearestPartition.bowPower >= 1 && !nearestPartition.equals(Country.countryList.get(ind)) && attacker.getId() != Country.countryList.get(ind).getCathcerId()) {
                    shouldRemove = true;

                    nearestPartition.bowPower = 0;
                }
    ...
    public static class Partition {
        ...
        public Color color;
        public ImageIcon partitionIcon;
        int bowPower = 0;
        int type;
...
//        Color partitionColor = new Color(colorArrayList.get(i)[0], colorArrayList.get(i)[1], colorArrayList.get(i)[2]);
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            NewV mapGenerator = new NewV(3 , true , 150 , 20);
...
//                }
            ...
            if(mapGenerator.isRandom) {
                mapGenerator.startAi();
                mapGenerator.initCountry = true;
                mapGenerator.initGame = true;
            }
    ...
    public class TimerPanel extends JPanel {
        ...
        public static final int INITIAL_TIME = 60; // Initial time in seconds
        public JLabel timerLabel;    public int currentTime;
        public TimerPanel() {
            setLayout(new BorderLayout());
            ...
            timer.start();
            currentTime = remainingTime;        updateTimerLabel();
        }
